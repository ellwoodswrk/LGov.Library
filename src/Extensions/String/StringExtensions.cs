...
namespace LGov.Library.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <returns>
        ///     the string up to the first occurence of the Separator, if the first characters are equal to the Separator then
        ...
        /// </summary>
        /// <param name="paragraph">The paragraph to search</param>
        /// <param name="wordSeparator">The word Separator</param>
        /// <returns>
        ///     the string up to the first occurence of the Separator, if the first characters are equal to the Separator then
        ///     an empty string will be returned
        /// </returns>
        ...
        {
            var words = paragraph.Split(wordSeparator.ToCharArray());
            return words[0];
        }
        ...
        /// </summary>
        /// <param name="input">the string of interest</param>
        /// <param name="replaceWith">the string to replace the unicode characters with, defaults to " "</param>
        /// <returns>
        ...
        /// <param name="paragraph">The paragraph to search</param>
        /// <returns>
        ///     String, if the last characters of the input paragraph is equal to the Separator then an empty string is
        ///     returned
        /// </returns>
        /// <remarks>Returns the last word from the paragraph using WordSeparator to terminate words</remarks>
        ...
        public static string LastWord(this string paragraph,
            string wordSeparator)
        {
            // if the Separator is too long we handle it here
            if (wordSeparator.Length > paragraph.Length)
            {
                return paragraph;
            }

            // now check that the paragraph doesn't end with a Separator
            // if it does then the last word is clearly null
            var sepLen = wordSeparator.Length;
            var paraLen = paragraph.Length;

            if (paragraph.Substring(paraLen - sepLen) == wordSeparator)
            {
                return "";
            }
        ...
        /// <param name="Separator">character string to be trimmed</param>
        /// <returns>string with any leading multiple Separators remvoved</returns>
        ...
        {
            // do nothing
            if (Separator.Length > textToTrim.Length)
            {
                return textToTrim;
            }

            if (Separator.Length == 0 || textToTrim.Length == 0)
            {
                return textToTrim;
            }
        ...
        /// <remarks>Returns the Nth word from the paragraph using WordSeparator to terminate words</remarks>
        ...
        /// <param name="replaceWith">The character(s) to replace the whitespace, defaults to CivicaParagraphSeparator</param>
        ...
        /// <param name="textToTrim">String to be trimmed</param>
        /// <param name="Separator">character string to be trimmed</param>
        /// <returns>string with any trailing multiple Separators remvoved</returns>
            ...
            string Separator)
        {
            ...
            while (result.Substring(result.Length - Separator.Length) == Separator)
            {
                if (result == Separator)
                {
                    result = "";
        ...
        /// <param name="textToTrim">String to be trimmed</param>
        /// <param name="Separator">character string to be trimmed</param>
        /// <returns>string with any leading or trailing multiple Separators remvoved</returns>
        ...
        /// <summary> Truncate </summary>
        /// <param name="paragraph">String to truncate</param>
        /// <param name="Separator">The word Separator to use, the first character is the only one considered to be Separators</param>
        /// <param name="length">Maximum number of characters to keep</param>
        /// <returns>String Array, Result[0] will contain the truncated paragraph, the remainder will be in Result[1]</returns>
        /// <remarks>
        ///     truncates a string so that words aren't cut in two, A word is defined
        ///     as a sequence of characters delimited by one or more Separators or the start or end of the string
        /// </remarks>
        public static string[] Truncate(this string paragraph,
            string Separator,
            int length)
        {
            var sep = Separator.ToCharArray();
        ...
        /// <summary> Truncate </summary>
        /// <param name="paragraph">String to truncate</param>
        /// <param name="Separator">The word Separator to use</param>
        /// <param name="length">Maximum number of characters to keep</param>
            ...
            int length)
        {
            ...
            if (paragraph == Separator.ToString())
            {
                result[0] = "";
                return result;
            }
            ...
            if (paragraph.WordCount(Separator) == 1)
            {
            ...
            // now we have the case where the char at position length is a Separator
            // if it is we have our split, same applies for position length -1

            var parArray = paragraph.ToCharArray();
            if (parArray[length] == Separator || parArray[length - 1] == Separator)
            {
                result[0] = paragraph.Substring(0,
                    length).RTrim(Separator.ToString());
                result[1] = paragraph.Substring(length,
                    paragraph.Length - length).LTrim(Separator.ToString());
                return result;
            }

            // we now know that the character at position length (or length-1) is not a Separator
            ...
            var lastOne = para1.LastWord(Separator.ToString());
            para2 = lastOne + para2;
        ...
        /// <summary>Simple word count function </summary>
        /// <param name="paragraph">The text to count the words of</param>
        /// <param name="Separator">The chars to use to distinguish between "words"</param>
        /// <returns>an integer indicating the number of words found</returns>
        /// <remarks>
        ///     This is a very simple count based on the number of Separators, if there is a
        ///     double Separator it will be ignored in the word count
        /// </remarks>
        public static long WordCount(this string paragraph,
            char[] Separator)
        {
