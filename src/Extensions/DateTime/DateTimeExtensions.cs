...
namespace LGov.Library.Extensions
{
    ...
    public static class DateTimeExtensions
    {
        ...
        /// <summary>
        ///     Calculate UK Easter Sunday date in any given year
        /// </summary>
        /// <param name="yr">
        ///     The year to check
        /// </param>
        /// <returns>
        ///     The date of UK easter sunday for the given year
        /// </returns>
        /// <remarks>
        /// </remarks>
        ...
        public static DateTime Prev1April(this DateTime dateToTest)
        {
            var result = dateToTest.Month >= 4
                ? new DateTime(dateToTest.Year, 4, 1, 0, 0, 0, DateTimeKind.Utc)
                : new DateTime(dateToTest.Year - 1, 4, 1, 0, 0, 0, DateTimeKind.Utc);
        ...
        public static bool? IsUkBankHoliday(this DateTime testDate)
        {
            try
            {
                ...
                if (testDate == new DateTime(2012, 5, 28, 0, 0, 0, DateTimeKind.Utc))
                    return false;

                if (testDate == new DateTime(2012, 6, 4, 0, 0, 0, DateTimeKind.Utc) || testDate == new DateTime(2012, 6, 5, 0, 0, 0, DateTimeKind.Utc))
                    return true;

                // bank holiday moved to VE day May 2020
                if (testDate == new DateTime(2020, 5, 8, 0, 0, 0, DateTimeKind.Utc))
                    return true;

                if (testDate == new DateTime(2020, 5, 4, 0, 0, 0, DateTimeKind.Utc))
                    return false;

                // first check for Easter
                if (testDate == EasterDate(testDate.Year).AddDays(-2))
                    return true;

                if (testDate == EasterDate(testDate.Year).AddDays(1))
                    return true;

                if (testDate.Month == 1)
                {
                    // it's january so check for new year
                    var dt = new DateTime(testDate.Year, 1, 1, 0, 0, 0, DateTimeKind.Utc);
                ...
                var dtXmas = new DateTime(testDate.Year, 12, 25, 0, 0, 0, DateTimeKind.Utc);
        ...
        /// <returns></returns>
        public static DateTime AddUkWorkingDays(this DateTime date, long days)
        {
            var current = date;
            var count = 0;
